import { createElement, createDualRangeSlider, makeValueEditable } from '../domHelpers.js';
...
export function ensureSoundDialogExists() {
	...
	if (!soundDialog) {
		...
		const { header, cleanup: headerCleanup } = createDraggableHeader(soundDialog, 'Manage Files and Buzzes');
		...
		limitsInfo.innerHTML = "Loading server limits...";
		...
		serverLabel.textContent = "Server files";
		...
		const exportsLabel = document.createElement("h3");
		exportsLabel.textContent = "Exported Buzzes";
		soundDialog.appendChild(exportsLabel);

		const exportsList = document.createElement("div");
		exportsList.id = "exportsList";
		soundDialog.appendChild(exportsList);
...
export function showFileManagerDialog(soundObj = null, onFileSelected = null) {
	const { dialog: soundDialog, serverList, exportsList, uploadInput, uploadStatus, limitsInfo } = ensureSoundDialogExists();
	...
	async function getMaxUploadSize() {
		...
		try {
			const data = await Backend.files.getMaxSize(Selectors.getWorkspaceId());
			maxSize = data.maxFileSize || "Not known";
			return maxSize;
		} catch (err) {
	...
	uploadInput.onchange = async (e) => {
		...
		if (file.size > maxSize) {
			...
			uploadStatus.innerHTML = `<strong>Error:</strong> File too large (${formatFileSize(file.size)}). Maximum: ${formatFileSize(maxSize)}`;
			uploadInput.value = '';
			return;
		}
	...
	async function refreshServerList(soundObj) {
		...
		try {
			...
			if (!files.length) {
				serverList.textContent = "No sound files on server.";
				return;
			}
	...
	async function refreshExportsList() {
		exportsList.textContent = "Loading...";

		try {
			const data = await Backend.files.listExports(Selectors.getWorkspaceId());
			const exports = data.exports || [];

			if (!exports.length) {
				exportsList.textContent = "No exported buzzes yet.";
				return;
			}
			...
			exports.forEach(exportInfo => {
				...
				const exportName = createElement("span", "server-list-filename");
				exportName.textContent = exportInfo.title || exportInfo.name;
				exportName.title = `${exportInfo.title || exportInfo.name}\nAuthor: ${exportInfo.author || 'Unknown'}\nCreated: ${exportInfo.created ? new Date(exportInfo.created).toLocaleString() : 'Unknown'}`;
				leftSection.appendChild(exportName);

				row.appendChild(leftSection);

				const exportSize = document.createElement("span");
				exportSize.textContent = exportInfo.sizeFormatted || formatFileSize(exportInfo.size);
				row.appendChild(exportSize);
				...
				deleteBtn.onclick = async (e) => {
					e.stopPropagation();
					const shouldDelete = await ModalSystem.confirm(`Are you sure you want to delete "${exportInfo.title || exportInfo.name}"?\n\nThis will permanently delete the entire buzz folder.\nThis action cannot be undone.`);
					if (shouldDelete) {
						try {
							const result = await Backend.files.deleteExport(Selectors.getWorkspaceId(), exportInfo.name);
							if (result.success) {
								refreshExportsList();
							} else {
								alert("Error deleting export: " + result.error);
							}
						} catch (err) {
			...
			exportsList.textContent = "Error: " + err.message;
	...
	(async () => {
		const maxSize = await getMaxUploadSize();
		limitsInfo.innerHTML = `Server limits: Max upload size: ${formatFileSize(maxSize)}`;
		refreshServerList(soundObj);
		refreshExportsList();
	})();
}
...
export async function showGridSampleDialog(soundObj, midiNote) {
	...
	modal.querySelector('.modal-body').appendChild(speedGateSlider);
	...
	holdSlider.max = 10;
	holdSlider.step = 0.1;
	holdSlider.value = existingSample.speedGateHold ?? (soundObj.params.speedGateHold ?? 0);
	...
	holdSlider.oninput = () => {
		...
		if (soundObj.params.gridSamples[midiNote]) {
			soundObj.params.gridSamples[midiNote].speedGateHold = val;
		}
	};
	...
	modal.querySelector('.modal-body').appendChild(holdSection);
	...
	return new Promise((resolve) => {
		...
		modal.querySelector('#gridLoadBtn').onclick = async () => {
			...
			if (fileSelected) {
				...
				const speedValues = speedGateSlider.getValues();
				const entry = {
				...
				if (holdTouched) entry.speedGateHold = parseFloat(holdSlider.value);
				soundObj.params.gridSamples[midiNote] = entry;
		...
		if (clearBtn) {
			clearBtn.onclick = async () => {
				...
				if (soundObj.params.gridSamples && soundObj.params.gridSamples[midiNote]) {
					...
					delete soundObj._eligibleGridKeys;
					if (soundObj._gridKeyHoldState) delete soundObj._gridKeyHoldState[midiNote];
					await changeSoundType(soundObj, 'Sampler');
				}
		...
		modal.querySelector('#gridCancelBtn').onclick = () => {
			...
			const speedValues = speedGateSlider.getValues();
			const holdVal = holdTouched ? parseFloat(holdSlider.value) : undefined;
			if (soundObj.params.gridSamples[midiNote]) {
				...
				if (holdTouched) soundObj.params.gridSamples[midiNote].speedGateHold = holdVal;
			} else if (newPitch !== 0 || speedValues.low !== 0 || speedValues.high !== 10 || holdTouched) {
				if (!soundObj.params.gridSamples) soundObj.params.gridSamples = {};
				const entry = {
					fileName: null,
					pitch: newPitch,
				...
				if (holdTouched) entry.speedGateHold = holdVal;
				soundObj.params.gridSamples[midiNote] = entry;
			}
			AppState.dispatch({ type: 'AUDIO_UPDATE_REQUESTED' });
			cleanup();
