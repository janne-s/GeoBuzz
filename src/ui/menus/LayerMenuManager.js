...
import { UIBuilder, createMenuStructure, createColorPicker } from '../controllers/UIBuilder.js';
...
import { getAvailableFXTypes } from '../../core/audio/SynthRegistry.js';

let AppState, Selectors, LayerManager, FXManager;
let closeAllMenus, createLayerEQNode, changeLayerFX, duplicateLayer, getEffectParameters;
...
export function setContext(context) {
	...
	createLayerEQNode = context.createLayerEQNode;
	changeLayerFX = context.changeLayerFX;
	duplicateLayer = context.duplicateLayer;
	getEffectParameters = context.getEffectParameters;
}

export function showLayerFXDialog(layer) {
	closeAllMenus();
	...
	const fxTabBtn = createButton('FX', () => showLayerFXTab(layer, container, 'fx', tabBar), '', { flex: '1' });
	fxTabBtn.dataset.tabId = 'fx';

	const eqTabBtn = createButton('EQ', () => showLayerFXTab(layer, container, 'eq', tabBar), '', { flex: '1' });
	eqTabBtn.dataset.tabId = 'eq';

	const gainTabBtn = createButton('Gain', () => showLayerFXTab(layer, container, 'gain', tabBar), '', { flex: '1' });
	...
	const closeBtn = createButton('Close', closeAllMenus, '', { width: '100%' });
...
export function showLayerFXTab(layer, container, tabId, tabBar) {
...
export function createLayerFXSlot(layer, slotNum, container, tabBar) {
	...
	if (currentFX.type !== 'none') {
		...
		getEffectParameters(currentFX.type).forEach(paramKey => {
			...
			content.appendChild(createParameterControl(PARAMETER_REGISTRY[paramKey], paramKey, layer, updateHeader, { small: true, slot: `slot${slotNum}`, paramName, isLayerFX: true }));
		});
	}
