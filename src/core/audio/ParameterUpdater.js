...
const audioNodePaths = new Map();

function getAudioNodeParameter(synth, audioNodePath) {
	if (!audioNodePath) return null;

	let parts = audioNodePaths.get(audioNodePath);
	if (!parts) {
		parts = audioNodePath.split('.');
		audioNodePaths.set(audioNodePath, parts);
	}
...
const PARAM_HANDLERS = {
	pitch: async (obj, value, isModulation) => {
	...
	frequency: async (obj, value, isModulation) => {
		const freq = value;
	...
	noiseType: (obj, value, isModulation) => {
		if (obj.synth?.noise) obj.synth.noise.type = value;
	},
	harmonicity: (obj, value, isModulation) => {
		if (obj.synth) {
	...
	modAttack: (obj, value, isModulation) => {
		if (obj.synth) {
	...
	modRelease: (obj, value, isModulation) => {
		if (obj.synth) {
	...
	modWaveform: (obj, value, isModulation) => {
		if (obj.synth) updatePartials(obj, true);
	},
	detune: (obj, value, isModulation) => {
		if (obj.synth) {
	...
	pulseWidth: (obj, value, isModulation) => {
		if (obj.synth) {
	...
	loopStart: (obj, value, isModulation) => {
		if (obj.type === "SoundFile" && obj.synth) applySoundFilePlaybackParams(obj, false);
	},
	loopEnd: (obj, value, isModulation) => {
		if (obj.type === "SoundFile" && obj.synth) applySoundFilePlaybackParams(obj, false);
	},
	loopFadeIn: (obj, value, isModulation) => {
		if (obj.type === "SoundFile" && obj.synth) applySoundFilePlaybackParams(obj, false);
	},
	loopFadeOut: (obj, value, isModulation) => {
		if (obj.type === "SoundFile" && obj.synth) applySoundFilePlaybackParams(obj, false);
	},
	...
	reverse: (obj, value, isModulation) => {
		if (obj.type === "SoundFile" && obj.synth) applySoundFilePlaybackParams(obj, true);
	},
	fadeIn: (obj, value, isModulation) => {
		if (obj.type === "SoundFile" && obj.synth) obj.synth.fadeIn = value;
	},
	fadeOut: (obj, value, isModulation) => {
		if (obj.type === "SoundFile" && obj.synth) obj.synth.fadeOut = value;
	},
	filterFreq: (obj, value, isModulation) => {
		if (obj.filter) obj.filter.frequency.rampTo(value, CONSTANTS.AUDIO_RAMP_TIME);
	},
	filterType: (obj, value, isModulation) => {
		if (obj.filter) obj.filter.type = value;
	},
	resonance: (obj, value, isModulation) => {
		if (obj.filter) obj.filter.Q.rampTo(value, CONSTANTS.AUDIO_RAMP_TIME);
	},
	attack: (obj, value, isModulation) => {
		if (obj.synth) {
	...
	sustain: (obj, value, isModulation) => {
		if (obj.synth) {
	...
	release: (obj, value, isModulation) => {
		if (obj.synth) {
	...
	modIndex: (obj, value, isModulation) => { if (obj.type === "FMSynth" && obj.synth?.modulationIndex) obj.synth.modulationIndex.value = value; },
	curveStrength: (obj, value, isModulation) => {
		if (!isModulation) {
			AppState.dispatch({ type: 'AUDIO_UPDATE_REQUESTED' });
		}
	},
	volume: (obj, value, isModulation) => {
	...
	partialCount: (obj, value, isModulation) => updatePartials(obj, false, { partialCount: value }),
	partialCurve: (obj, value, isModulation) => updatePartials(obj, false, { partialCurve: value }),
	modPartialCount: (obj, value, isModulation) => updatePartials(obj, true, { modPartialCount: value }),
	modPartialCurve: (obj, value, isModulation) => updatePartials(obj, true, { modPartialCurve: value }),
	polyphony: (obj, value, isModulation) => {
...
export function updateSynthParam(obj, param, value, options = {}) {
	...
	try {
		...
		if (handler) {
			handler(obj, value, isModulation);
